...
where lastIvl >= 21 and id > ?"""+lim, (self.col.sched.dayCutoff-86400)*1000)
    ...
    def _dueInfo(self, tot, num):
        i = []
        self._line(
            i, _("Total"), ngettext("%d review", "%d reviews", tot) % tot)
...
group by day order by day""" % (self._limit(), lim),
    ...
    def repsGraph(self):
        if self.type == 0:
            ...
            days = None
            chunk = 30
        return self._repsGraph(self._done(days, chunk), days,
                               _("Review Count"), _("Review Time"))
    ...
    def _repsGraph(self, data, days, reptitle, timetitle):
        ...
        if self.type == 0:
            ...
            convHours = True
        txt += self._title(
            timetitle, _("The time taken to answer the questions."))
    ...
    def _splitRepData(self, data, spec):
        ...
        for (n, col, lab) in spec:
            if len(totd[n]) and totcnt[n]:
                ...
                # lines
                ret.append(dict(data=totd[n], color=col, label=None, yaxis=2,
...
group by day order by day)""" % lim, cut=self.col.sched.dayCutoff)
    ...
    def ivlGraph(self):
        ...
        txt += self._graph(
            id="ivl", ylabel2=_("Percentage"),
            data=[dict(data=ivls, color=colIvl),
                  dict(data=totd, color=colCum, yaxis=2, bars={'show': False},
                       lines=dict(show=True), stack=False)],
            conf=dict(xaxis=dict(min=-0.5, max=ivlmax + 0.5),
...
select count(), avg(ivl), max(ivl) from cards
where did in %s and queue = 2""" % self._limit()))
    ...
    def easeGraph(self):
        ...
        txt += self._graph(
            id="ease",
            data=[dict(data=d['lrn'], color=colLearn, label=_("Learning")),
    ...
    def _easeInfo(self, eases):
        ...
        for type in range(3):
            ...
            try:
                ...
                pct = 0
            i.append(_("Correct: <b>%(pct)0.2f%%</b><br>(%(good)d of %(tot)d)")
                     % dict(pct=pct, good=good, tot=tot))
        return ("""
<center><table width=%dpx><tr><td width=50></td><td align=center>"""
                % self.width + "</td><td align=center>".join(i) +
                "</td></tr></table></center>")
...
(case when type in (0,2) and ease = 4 then 3 else ease end),
count() from revlog %s
...
order by thetype, ease""" % lim)
    ...
    def _hourRet(self):
        ...
        if pd:
            lim += " and id > %d" % (
                (self.col.sched.dayCutoff - (86400 * pd)) * 1000)
        return self.col.db.all("""
select
...
$(function () {
    ...
    if (conf.series.pie) {
        conf.series.pie.label.formatter = function(label, series){
            return '<div class=pielabel>' +
                   Math.round(series.percent)+'%%</div>';
        };
    }
    $.plot($("#%(id)s"), %(data)s, conf);
});
</script>""" % dict(id=id, w=width, h=height, ylab=ylabel, ylab2=ylabel2,
    ...
    def _avgDay(self, tot, num, unit):
        vals = []
        try:
            vals.append(_("%(a)0.1f %(b)s/day") % dict(a=tot / float(num),
                                                       b=unit))
            return ", ".join(vals)
