...
select distinct(n.id) from cards c, notes n where c.nid=n.id and """ + preds
    ...
    def _findTag(self, xxx_todo_changeme):
        (val, args) = xxx_todo_changeme
        if val == "none":
            return 'n.tags = ""'
        val = val.replace("*", "%")
    ...
    def _findCardState(self, xxx_todo_changeme1):
        (val, args) = xxx_todo_changeme1
...
(c.queue = 1 and c.due <= %d)""" % (
    ...
    def _findRated(self, xxx_todo_changeme2):
        # days(:optional_ease)
        (val, args) = xxx_todo_changeme2
        r = val.split(":")
        try:
            days = int(r[0])
        except ValueError:
    ...
    def _findAdded(self, xxx_todo_changeme3):
        (val, args) = xxx_todo_changeme3
        try:
            days = int(val)
        except ValueError:
            return
    ...
    def _findProp(self, xxx_todo_changeme4):
        # extract
        (val, args) = xxx_todo_changeme4
        m = re.match("(^.+?)(<=|>=|!=|=|<|>)(.+?$)", val)
        if not m:
            return
    ...
    def _findNids(self, xxx_todo_changeme5):
        (val, args) = xxx_todo_changeme5
        if re.search("[^0-9,]", val):
            return
        return "n.id in (%s)" % val

    def _findCids(self, xxx_todo_changeme6):
        (val, args) = xxx_todo_changeme6
        if re.search("[^0-9,]", val):
            return
        return "c.id in (%s)" % val

    def _findMid(self, xxx_todo_changeme7):
        (val, args) = xxx_todo_changeme7
        if re.search("[^0-9]", val):
            return
        return "n.mid = %s" % val

    def _findModel(self, xxx_todo_changeme8):
        (val, args) = xxx_todo_changeme8
        ids = []
        val = val.lower()
    ...
    def _findDeck(self, xxx_todo_changeme9):
        # if searching for all decks, skip
        (val, args) = xxx_todo_changeme9
        if val == "*":
            return "skip"
        # deck types
        elif val == "filtered":
    ...
    def _findTemplate(self, xxx_todo_changeme10):
        # were we given an ordinal number?
        (val, args) = xxx_todo_changeme10
        try:
            num = int(val) - 1
        except:
            num = None
...
where mid in %s and flds like ? escape '\\'""" % (
    ...
    def _findDupes(self, xxx_todo_changeme11):
        # caller must call stripHTMLMedia on passed val
        (val, args) = xxx_todo_changeme11
        try:
            mid, val = val.split(",", 1)
        except OSError:
            return
